<div>
    <h1 class="text-xl font-semibold text-gray-900 mb-6">Programas</h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($programas as $programa)
                    <tr wire:key="programa-{{ $programa->id }}" class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-gray-900">{{ $programa->nombre }}</td>
                        <td class="px-4 py-3">
                            <span @class([
                                'inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium',
                                'bg-emerald-50 text-emerald-700' => $programa->activo,
                                'bg-gray-100 text-gray-500' => ! $programa->activo,
                            ])>
                                {{ $programa->activo ? 'Activo' : 'Inactivo' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-right space-x-4">
                            <button wire:click="editar({{ $programa->id }})" class="text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors duration-150">Editar</button>
                            <button wire:click="alternarActivo({{ $programa->id }})" class="text-sm font-medium text-gray-500 hover:text-gray-700 transition-colors duration-150">
                                {{ $programa->activo ? 'Desactivar' : 'Activar' }}
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-12 text-center text-sm text-gray-500">Sin programas registrados.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
