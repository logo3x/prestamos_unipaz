<div>
    <h1 class="text-xl font-semibold text-gray-900 mb-6">Sedes</h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($sedes as $sede)
                    <tr wire:key="sede-{{ $sede->id }}" class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-gray-900">{{ $sede->nombre }}</td>
                        <td class="px-4 py-3">
                            <span @class([
                                'inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium',
                                'bg-emerald-50 text-emerald-700' => $sede->activo,
                                'bg-gray-100 text-gray-500' => ! $sede->activo,
                            ])>
                                {{ $sede->activo ? 'Activa' : 'Inactiva' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-right space-x-4">
                            <button wire:click="editar({{ $sede->id }})" class="text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors duration-150">Editar</button>
                            <button wire:click="alternarActivo({{ $sede->id }})" class="text-sm font-medium text-gray-500 hover:text-gray-700 transition-colors duration-150">
                                {{ $sede->activo ? 'Desactivar' : 'Activar' }}
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-12 text-center text-sm text-gray-500">Sin sedes registradas.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
